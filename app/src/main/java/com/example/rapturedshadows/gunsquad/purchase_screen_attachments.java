...
/**
 * Created by RapturedShadows on 12/16/16.
 */

public class purchase_screen_attachments extends AppCompatActivity {
    ...
    public void populateList() {
        ArrayAdapter<Unit> adapter = new purchase_screen_attachments.MyListAdapter();
    ...
    public class MyListAdapter extends ArrayAdapter<Unit> {
        public MyListAdapter() {
            super(purchase_screen_attachments.this, R.layout.solder_view_for_purchase, allUnits);
        }

        public View getView(final int position, final View convertView, final ViewGroup parent){

            final Attachments attach = (Attachments) getIntent().getSerializableExtra("gun");
            ...
            primary.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    currentUnit.setPrimaryAttachment(attach,attach.getTypeID());
            ...
            secondary.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (currentUnit.getSecondary().getGunID() != 0){
                        currentUnit.setSecondaryAttachment(attach,attach.getTypeID());
                        currentUnit.totalUpStats();
                        Toast.makeText(getApplicationContext(), "Equipped", Toast.LENGTH_SHORT).show();
                        onBackPressed();
                    } else
                        Toast.makeText(getApplicationContext(), "Cannot Equip to Pistol", Toast.LENGTH_SHORT).show();
                }
            });
